...
M.setup = function()
    local config_dir = vim.fn.stdpath('config') .. '/lua/plugs'
    -- plugins do not need to load, NOTE: no .lua suffix required
    local unload_plugins = {"init", -- we don't need to load init again
    "vgit", "nvim-hlslens", "auto-session", "vim-illuminate", "vista", "symbols-outline", "nvim-gps", "tabout",
                            "hlargs", "spellsitter", "tabnine", "startup", "nvim-neoclip", "nvim-biscuits",
                            "nvim_context_vt", "filetype"}
    ...
    for _, fname in pairs(vim.fn.readdir(config_dir)) do
        ...
        elseif vim.fn.isdirectory(fname) then
